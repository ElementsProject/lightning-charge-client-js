...
var debug = require('debug')('lightning-kite-client');

var enc = encodeURIComponent;

var LightningKiteClient = function () {
  function LightningKiteClient(url, token) {
    _classCallCheck(this, LightningKiteClient);

    this.req = (0, _superagentBaseuri2.default)(url);
    token && this.req.use(function (r) {
      return r.auth('api-token', token);
    });
  }

  _createClass(LightningKiteClient, [{
    key: 'invoice',
    ...
    value: function wait(invoice_id) {
      ...
      return this.req.get('/invoice/' + enc(invoice_id) + '/wait?timeout=' + +timeout).then(function (res) {
        ...
        return err.status === 402 ? null // 402 Payment Required: time-out reached without payment
  ...
  return LightningKiteClient;
}();

module.exports = function (url, token) {
  return new LightningKiteClient(url, token);
};